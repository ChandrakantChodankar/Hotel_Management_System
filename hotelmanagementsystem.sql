...
--
-- Host: 127.0.0.1
-- Generation Time: Feb 21, 2023 at 03:25 PM
...
--
-- Stand-in structure for view `works`
...
--
-- Structure for view `works`
--
DROP TABLE IF EXISTS `works`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `works`  AS SELECT `employee`.`employee_id` AS `employee_id`, `employee`.`employee_name` AS `employee_name`, `room`.`roomno` AS `roomno`, `room`.`roomtype` AS `roomtype` FROM (`employee` join `room`) WHERE `employee`.`roomno` = `room`.`roomno``roomno`  ;
